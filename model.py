...
class Model(torch.nn.Module):
    ...
    def block_sim(self, z1: torch.Tensor, z2: torch.Tensor, random=True):
        ...
        for i in range(block_num):
            ...
            block_sim_list.append(torch.mm(z1_block, z2_block.t()))
        return block_sim_list
    def intro_block_distributionwoaug(self, datax, dataedge_index,databatch):    
        z1 = self.forward(datax, dataedge_index, databatch)
        h1 = self.projection(z1)
        between_block_sim_list = self.block_sim(h1, h1)
        return between_block_sim_list
    ...
    def batched_semi_block_loss(self, x_1, edge_index_1, batch1, x_2, edge_index_2, batch2, batch_size: int, mean: bool):
        device = x_1.device
        ...
        for i in range(num_batches):
            ...
            z1 = self.forward(x_1, edge_index_1, batch1)[0]
            z2 = self.forward(x_2, edge_index_2, batch2)[0]
            z1 = self.projection(z1)
            z2 = self.projection(z2)

            z1_batch = z1[mask]

            refl_block_sim_list = self.block_sim(z1_batch, z1)
            between_block_sim_list = self.block_sim(z1_batch, z2)
            ...
            for j in range(block_num):
                refl_block_sim += f(refl_block_sim_list[j])  # [B, N]
                between_block_sim += f(between_block_sim_list[j])  # [B, N]

            diag = between_block_sim[:, i * batch_size:(i + 1) * batch_size].diag()
            refl_diag = refl_block_sim[:, i * batch_size:(i + 1) * batch_size].diag()

            loss = -torch.log(diag / (refl_block_sim.sum(1) + between_block_sim.sum(1) - refl_diag))
